...
//-----------------------------------------------------------------------------
    ...
    function search($filters, $per_page = NULL, $offset = NULL)
    {
        ...
        //Construir consulta
            $this->db->select('events.id, events.type_id, start, end, seconds, events.status, events.element_id, user_id, users.display_name');
        ...
        if ( is_null($per_page) )
        {
            $query = $this->db->get('events'); //Resultados totales
        } else {
    ...
    function role_filter()
    {
        ...
        $condition = 'events.id = 0';  //Valor por defecto, ningún user, se obtendrían cero users.
